...
## 스크롤 큐(↓)를 표기 규칙 확인 없이 복원 (2026-09-24, a1b3147)
- **실패**: 대표가 "히어로 화살표가 안 보인다"고 하자 `ddd3f50`에서 뺀 `s-void__scroll`을 되살려 배포했다. CLAUDE.md 브랜드 어조 절의 **표기 규칙 "스크롤 큐(`↓`) 금지"(2026-07-05 확립)**를 확인하지 않았다. 제거 커밋 메시지(taste-skill anti-slop)만 보고 "대표가 원하면 복원"으로 판단했다.
- **원인**: Paper 시안에 화살표가 남아 있어 "시안과 사이트 불일치 = 사이트 누락"으로 읽었다. 실제로는 사이트 쪽이 규칙을 따른 것이고 시안이 낡은 것이었다.
- **방지**: 요소를 **되살리기 전에** 그 요소를 뺀 이유를 CLAUDE.md 표기 규칙·브랜드 금지 목록에서 먼저 찾는다(`grep -n "금지" CLAUDE.md`). 규칙과 충돌하면 복원 전에 대표에게 규칙 쪽을 바꿀지 묻는다. Paper와 사이트가 다를 때 어느 쪽이 정본인지는 규칙으로 판정한다.
- **결과**: 대표가 규칙 쪽을 바꾸기로 결정(2026-09-24) — CLAUDE.md 표기 규칙을 "히어로 하단 중앙 1곳만 허용"으로 개정하고 화살표 유지.

## 프랑스어 문구에 둥근 아포스트로피(’)를 넣으면 셀렉터·테스트가 깨질 수 있다 (2026-09-24)
- fr 사전의 `'`를 `’`로 일괄 치환한 뒤, 연령 게이트 버튼을 `"Oui, j'ai 19 ans ou plus"`처럼 곧은 따옴표 이름으로 찾던 브라우저 스크립트가 버튼을 못 찾았다. 접근성 이름도 텍스트를 따라 바뀐다.
- **방지**: 버튼·링크를 이름으로 찾을 때는 로케일 문구를 하드코딩하지 말고 스냅샷 ref나 정규식(`/^Oui/`)을 쓴다.

## 2026-09-26 — 화면 상단 브랜드 표기를 글자로 둠

- **실패 모드**: 소유 관리(03A·03B·03C·03D) 헤더 가운데에 브랜드를 `MUSE DE MARÉE` 텍스트로 두었다. 대표 지적 — "이런 기본적인 것도 안 지킬래".
- **감지 신호**: 헤더·히어로·메일 머리에 브랜드명이 `<span>`/Text 노드로 들어가 있다.
- **방지 규칙**: 화면 머리의 브랜드 자리는 항상 로고 이미지다. 어두운 바탕 = `logo_text_trans_W.png`(워드마크) 또는 `logo_trans_W_lg.png`(심볼), 밝은 바탕 = `logo_text_trans.png`. 글자 표기는 카피라이트 줄(© 2026 MUSE DE MARÉE)에만 허용. 새 화면을 만들거나 고칠 때 Paper와 코드 양쪽에서 `MUSE DE MAR` 텍스트를 검색해 확인한다.

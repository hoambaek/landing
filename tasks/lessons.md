...
## 소유권 이전 폐기 — 위 항목 무효 (2026-07-25, db6b312)
- **바로 위 phase-3 항목의 마지막 줄("소유권 이전은 되돌릴 수 없음…")은 폐기된 기능 설명이다.** 이전은 기능째 삭제됐고 `/b`에 `transfer` 문자열 0건. 남은 소유자 액션은 4개: `requestOwnerOtp`·`verifyOwnerOtp`·`signOutOwner`·`updateOwnerName`. OTP 인증·HMAC 세션 자체는 그대로 유효하다.
- 폐기 계기: **"이름 수정"이 사실상 소유권 이전이었다.** `updateOwnerInfo`가 email까지 덮어써서 OTP 수신자가 바뀌면 동의도 감사 기록도 없이 소유자가 갈린다 → 액션을 `updateOwnerName`으로 좁히고 이메일은 읽기 전용(변경은 문의 경로로만).
- 감지 신호: 인증 수단(OTP 수신 주소)을 **그 인증으로 보호되는 화면 안에서** 바꿀 수 있으면 권한 상승 경로다. "수정"과 "이전"의 경계는 기능 이름이 아니라 **바뀌는 필드가 인증 앵커인지**로 갈린다.

## tasks/todo.md는 시간순 로그 — 현재 상태가 아니다 (2026-07-25)
- **실패**: "NFC 작업 다 됐나?" 질문에 todo.md의 옛 절을 읽고 미결 2건(소유권 이전 미검증·`ownTitle` 소유격 위반)을 보고했다. **둘 다 이미 닫혀 있었다** — 이전은 폐기됐고 `ownTitle`은 같은 날 `N° {serial}의 기록에`로 바뀐 뒤였다.
- **원인**: todo.md는 세션마다 아래로 덧붙는 로그라 **뒤쪽 절이 앞쪽을 무효화한다.** "### 미결"·"phase-N 대기" 같은 헤딩은 *그 시점의* 미결이지 지금의 미결이 아니다.
- **방지**: 상태를 묻는 질문엔 **코드를 먼저 본다.** 기능 존재 여부는 `grep -rn <심볼> src/`, 카피 값은 `copy.ts` 실값, 미결 항목은 `git log -S'<옛 문자열>'`로 그 뒤에 바뀌었는지 확인한 다음 보고한다. 문서는 *왜* 그렇게 됐는지의 근거로만 쓴다.
- 부수 효과: 폐기된 기능이 PR #1 본문에도 그대로 실려 나갔다. 문서를 원본으로 삼으면 **오류가 하위 산출물로 전파된다.**

...
## 2026-09-26 — NFC 입장 페이지(/b) 히어로 필름 연결

- [x] 32초 세로 필름 제작 (입수 → 바다의 1년 → 인양 → 로고). 소스·빌드 스크립트: `~/Documents/Cursor/mdm-image-work/nfc-hero-video/` (`build_final.py`, 마스터 `final_master.mp4`)
  - 실사: 다이버·로프 컷(`etc.mp4`) / AI(Seedance 2.5 초안 + bytedance 업스케일 1080p): 입수 2컷(실사 스틸 기반)·타임랩스·해저·로프 매듭·기포·인양
  - 효과음: Sonilo 구간별 foley, -20 LUFS / 월 표기 `2026. 01 → 12`와 로고(`logo_intro_W.png`)는 영상에 구워 넣음
- [x] `public/videos/nfc-entry-film.mp4` (H.264 CRF27, 8.5MB, 28초 — 2026-09-26 대표 지시로 끝 로고 구간 제거, 32초판은 작업 폴더 `web_crf27.mp4`) → `ENTRY_VIDEO_SRC` 연결
- [x] 필름 재생 중 페이지 로고·재생 버튼 숨김, 재생 버튼은 화면 전체 투명 탭 영역(건너뛰기)으로
- [x] 좌상단 사운드 토글(무음 시작, 44px 터치 영역). 시트가 올라오면 음소거 + 토글 숨김
- [x] `filmMeta` 00:08 → 00:28 (5개 언어)
- [x] 한 번만 재생(loop 제거, 2026-09-26 대표 지시): `onEnded`로 시트 등판 → 시트 위 필름 조각 가운데에 다시 보기 버튼(조각 높이를 재서 배치) → 누르면 시트 내려가고 0초부터 재생. 건너뛰기로 시트를 올리면 필름 일시정지
  - 이 전환으로 길이 기반 타이머 예약(`scheduleSheetUp`)과 하이드레이션 우회 코드는 제거됨
- 검증: tsc·eslint·`pnpm build` 통과. 로컬 임시 라우트(삭제함)에서 aside로 확인 — loop=false, 탭 건너뛰기(시트 등판·일시정지·다시 보기 버튼 조각 중앙), 다시 보기(시트 내려감·재생), 28s 종료(ended·시트 등판·버튼 표시)
- ⚠️ 후속: 2027년 입수분이 생기면 영상 속 `2026` 월 표기가 틀린 정보가 된다 → 월 표기를 코드 오버레이(배치 입수·인양 월)로 옮길 것
- ⚠️ DB에 미등록 병 코드가 없어 실제 `/b/{code}` 경로로는 필름 화면을 확인하지 못함(등록된 병은 01B로 감)

## 2026-09-26 — NFC 필름 화면 리디자인 반영 (Paper 01 v2-가·나 · 01B · 01C-가·나 · SPEC B v2 · SPEC C)

- [x] 입장 필름 재생 중: 하단 그라데이션(`.filmFade`) 제거, 영상 위에는 우상단 사운드 칩 하나만
- [x] 언어 선택: 필름 위 → 등록 시트 첫 입력 행 오른쪽(밝은 종이용 칩·패널, SPEC B v2). 라틴 지면(en·fr)은 첫 행이 두 칸이라 칸 위 한 줄로
- [x] 다시 보기 버튼: 안전영역 아래 남은 필름 조각의 가운데(`--strip`)
- [x] 01B: "▸ 필름 보기" 링크 → 01C(`BottleFilm.tsx`) 화면 전체 재생, 소리 켜고 시작(거절 시 무음), 좌상단 닫기·우상단 사운드·Esc
- [x] 01C-나: 끝나면 다시 보기 + "돌아가기"(× 걷음)
- [x] 문구 `claimedFilm`·`filmBack` 5개 언어
- 검증: tsc·eslint·build 통과, 로컬 임시 라우트(삭제함)에서 aside로 재생·건너뛰기·언어 전환(ko·fr)·01B→01C→종료→돌아가기 확인

## 2026-09-26 — 01B·02 링크 정리, 03 로고, 확인 코드 메일 개선

- [x] 01B: 「소유자이신가요? 본인 인증」 제거(소유 관리는 기록 페이지 → 소유 정보 관리), 「필름 보기」 → 「필름 다시 보기」(5개 언어)
- [x] 02: 「인증서 전체 보기」 제거(인증서 화면에서 돌아올 길이 없었다). 문구 키 claimedVerify·inscribedCertLink 삭제
- [x] 03 기록 히어로: 상단 가운데 브랜드 심볼 64px(Paper 03 Hero Logo), 스크롤 디졸브에 함께 태움
- [x] 확인 코드 메일: 코드의 공백 문자 제거(3+3 끊음은 여백으로) + user-select:all → 길게 누르기로 여섯 자리 한 번에 선택
- [x] 확인 코드 메일 → **확인 링크 중심**(대표 지시): 「본인 인증하기」 버튼이 주인공, 코드는 「다른 기기에서 보고 계시면」 보조. 제목 「소유자 확인 링크」
  - 링크 `/b/{nfc}/owner?otp=…` → 03B가 뜨자마자 서버 액션으로 스스로 인증(GET 미리 열기로는 소모 안 됨), 주소창 코드 즉시 제거, 실패 시 코드 채운 03B에 오류
- 검증: tsc·eslint·build 통과, 로컬에서 N° 2(Jt8F5j45)로 01B·기록 로고·링크 자동 인증(틀린 코드로 실패 경로)·주소 정리 확인. 성공 경로와 메일 HTML은 실제 발송으로 확인 필요
- ⚠️ 남은 것: 기록 페이지의 「인증서 보기」도 돌아올 길 없는 인증서 화면으로 간다 — 인증서 화면에 뒤로 가기 필요
